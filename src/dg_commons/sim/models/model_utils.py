...
def apply_force_limits(force: float, p: RocketParameters):
    ...
    if force <= p.F_limits[0]:
        force = 0
        logger.debug(
            f"Negative force provided, force set to 0"
        )
    elif force >= p.F_limits[1]:
        force = p.F_limits[1]
        logger.debug(
            f"Reached max force, force set to {force:.2f}: \nforce {force:.2f}\tforce limits [{p.F_limits[0]:.2f},{p.F_limits[1]:.2f}]"
        )
    return force
...
def apply_ang_vel_limits(ang_vel: float, p: RocketParameters):
    ...
    if ang_vel <= p.dphi_limits[0]:
        ...
        logger.debug(
            f"Min angular velocity reached, angular velocity set to {ang_vel:.2f}: \nangular velocity {ang_vel:.2f}\tangular velocity limits [{p.dphi_limits[0]:.2f},{p.dphi_limits[1]:.2f}]"
        )
...
def apply_ang_constraint(ang: float, dang: float, p: RocketParameters):
    ...
    if (ang <= p.phi_limits[0] and dang < 0) or (ang >= p.phi_limits[1] and dang > 0):
        dang = 0
        logger.debug(
            f"Reached min or max angle, acceleration set to {dang:.2f}: \nangle {ang:.2f}\tangle limits [{p.phi_limits[0]:.2f},{p.phi_limits[1]:.2f}]"
        )
    return dang
