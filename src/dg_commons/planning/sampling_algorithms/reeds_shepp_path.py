...
def reeds_shepp_path_planning(sx, sy, syaw, gx, gy, gyaw, maxc, step_size):
    paths = calc_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size)
    if not paths:
        return None, None, None  # could not generate any path

    # search minimum cost path
    best_path_index = paths.index(min(paths, key=lambda p: abs(p.L)))
    b_path = paths[best_path_index]
    path = []
    for idx, x in enumerate(b_path.x):
        path.append(SE2Transform(p=[b_path.x[idx], b_path.y[idx]], theta=b_path.yaw[idx]))
    return path, b_path.ctypes, b_path.lengths