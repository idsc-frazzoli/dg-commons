...
from dg_sim import CollisionReport, SimLog
from dg_sim.collision_utils import velocity_of_P_given_A
from dg_sim.simulator_structures import LogEntry
...
def plot_collision(collision_report: CollisionReport, sim_log: SimLog):
    ...
    for i, (player, p_report) in enumerate(collision_report.players.items()):
        ...
        plt.arrow(xc, yc, vel[0], vel[1], width=arr_width, head_width=head_width, ec=col_before, fc=col_before,
                  alpha=.8,
                  zorder=_Zorders.VEL_BEFORE)
        plt.arrow(xc, yc, vel_after[0], vel_after[1], width=arr_width, head_width=head_width, ec=col_after,
                  fc=col_after,
        ...
        plt.text(xc, yc + 4 * arrow_shift, f"{p_report.velocity[1]:.3f}", horizontalalignment="center",
                 verticalalignment="center", zorder=_Zorders.VEL_BEFORE, color=col_before)
        plt.text(xc, yc + 2 * arrow_shift, f"{p_report.velocity_after[1]:.3f}", horizontalalignment="center",
        ...
        plt.arrow(imp_point[0], imp_point[1], vel_atP[0], vel_atP[1], width=arr_width, head_width=head_width,
                  ec=p_color,
