...
def sPolygon2crPolygon(shapely_polygon: Polygon) -> CommonRoadPolygon:
    ...
    return CommonRoadPolygon(vertices, 0.125, 20)


def test_visibility_filter():
    vis = ShapelyViz()
    sensor_pose: SE2Transform = SE2Transform(p=[5, -10], theta=1.57)
    ...
    sensor_view: Polygon = Polygon(lidar2d.view_vertices(obs))
