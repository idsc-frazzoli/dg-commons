...
from dg_commons.sim.models.diff_drive import (
    DiffDriveState,
    DiffDriveModel,
    DiffDriveCommands,
)
...
def get_simple_scenario() -> SimContext:
    ...
    x0_diffdrive = DiffDriveState(x=20, y=5, psi=deg2rad(45))
    ...
    models = {
        ...
        Ego2: DiffDriveModel.default(x0_diffdrive),
    ...
    diffdrive = DgSampledSequence[DiffDriveCommands](
        timestamps=[0, 1, 2, 3, 4, 5],
        values=[
            DiffDriveCommands(omega_l=10, omega_r=10),
            DiffDriveCommands(omega_l=2, omega_r=2),
            DiffDriveCommands(omega_l=1, omega_r=-0.5),
            DiffDriveCommands(omega_l=3, omega_r=+0.4),
            DiffDriveCommands(omega_l=5, omega_r=-1),
            DiffDriveCommands(omega_l=0, omega_r=-3),
        ],
    )
    ...
    static_obstacles = [
        StaticObstacle(boundaries),
        StaticObstacle(poly1),
        StaticObstacle(poly2),
    ]
    players = {Ego: NPAgent(moving_vehicle), Ego2: NPAgent(diffdrive), DObs1: NPAgent(dyn_obstacle_commands)}
